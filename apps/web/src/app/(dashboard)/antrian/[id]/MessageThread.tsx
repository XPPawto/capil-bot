    ...
    <div className="flex flex-col gap-3 rounded-xl border border-line bg-surface p-4">
          ...
          <div key={m.id} className={`flex flex-col ${m.direction === "OUTBOUND" ? "items-end" : "items-start"}`}>
            <div
              className={`max-w-[80%] whitespace-pre-wrap rounded-lg px-3 py-2 text-sm ${
                m.direction === "OUTBOUND" ? "bg-pastel-green text-pastel-green-ink" : "bg-canvas text-ink"
              }`}
            >
              {m.message}
              {m.hasAudio && (
      ...
      <div className="flex gap-2 border-t border-line pt-3">
        ...
        <button
          ...
          className="self-end rounded-md bg-brand px-4 py-2 text-sm font-medium text-brand-ink transition-colors hover:bg-brand-hover disabled:opacity-50"
        >
          Kirim
        </button>
      </div>
