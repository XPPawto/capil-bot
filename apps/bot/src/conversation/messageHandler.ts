...
import { isHumanTakeoverActive, logInboundIfActiveRequest } from "./humanTakeover";
 ...
 * dari situ dulu supaya waNumber yang disimpan adalah nomor HP sungguhan, bukan ID
...
export async function handleIncomingMessages(sock: WASocket, payload: MessagesUpsertPayload): Promise<void> {
  ...
  for (const msg of payload.messages) {
    ...
    if (!jid) continue;
    // "@newsletter" = Channel WhatsApp (siaran satu-arah), broadcast/status juga bukan
    // percakapan dengan orang tertentu - tidak relevan buat alur bot.
    if (jid === "status@broadcast" || jid.endsWith("@broadcast") || jid.endsWith("@newsletter")) continue;
    // Alur bot (menu, pengumpulan syarat, dst) SENGAJA TIDAK PERNAH dijalankan untuk pesan
    // grup - risiko privasi nyata kalau alur pengajuan dokumen pribadi (KTP dkk) berjalan
    // di konteks publik/semi-publik grup.
    if (jid.endsWith("@g.us")) continue;
    // Pesan yang KITA kirim sendiri (bot otomatis maupun balasan manual dari HP) bukan
    // input warga, tidak perlu diproses lewat alur bot.
    if (msg.key.fromMe) continue;
    if (!msg.message) continue;
    // protocolMessage (edit/hapus) & reactionMessage bukan pesan baru - tidak punya teks
    // yang berarti buat alur bot, kalau lolos ke bawah bisa keliru dianggap "pesan kosong".
